...
const LabSignUp = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    confirmPassword: '',
    employeeId: '',
    labDepartment: 'general',
    certificationNumber: '',
    specialization: '',
    shiftPreference: 'morning',
    canHandleSamples: true
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.certificationNumber) newErrors.certificationNumber = 'Certification number is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => navigate('/UploadResults'), 2000);
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="employee-signup-form">
          ...
          <div className="form-group">
            ...
            {errors.certificationNumber && (
              <span className="error-message">{errors.certificationNumber}</span>
            )}
          </div>
